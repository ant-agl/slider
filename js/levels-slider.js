...
function toLevel(itemIndex, data = false) {
  ...
  if ($(window).outerWidth() > 767) {
    if ($(".levels-slider__content").scrollLeft() != wContent * slide)
      $(".levels-slider__content").animate(
        { scrollLeft: wContent * slide },
        400,
        "linear"
      );
  } else {
    ...
    $(".levels-slider__content").animate(
      { scrollLeft: wBlock * itemIndex - wContent / 2 + wBlock / 2 },
      200,
      "linear"
    );
  }
...
function updateData(itemIndex, data = false) {
  ...
  if (level.more == "")
    $(".level-more").closest(".loyality-program__more-block").hide();
