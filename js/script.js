...
$.getJSON('js/levels-data.json', data => {
  ...
  data.forEach((level, i) => {
    ...
    if (i % ITEMS_IN_SLIDE == 0) {
      dotsHtml += `
        <div class="levels-slider__dot ${i == 0 ? 'active' : ''}" data-index="${parseInt(i / ITEMS_IN_SLIDE)}"></div>
      `;
    }
  });
  ...
  if ($(window).outerWidth() > 767)
    $('.levels-slider__content > *').css('min-width', wSliderContent / ITEMS_IN_SLIDE);
  else 
    $('.levels-slider__content > *').css('min-width', 80);
  ...
  let bLine = ($('.level-block__level').outerHeight() - $('.level-block__line').outerHeight()) / 2 + 5;
